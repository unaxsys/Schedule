...
function buildSegments({ baseDateISO, intervals = [] }) {
  ...
  source.forEach((interval) => {
    ...
    const dayParts = calcDaySplit([start, end], baseDateISO);
    if (dayParts.length) {
      segments.push(...dayParts);
    }
  });
...
const NON_WORKING_SHIFT_CODES = new Set(['', 'P', 'O', 'B', 'R', 'OFF', 'REST', 'LEAVE']);

function isRealWorkingShiftCode(shiftCode) {
  const normalized = String(shiftCode || '').trim().toUpperCase();
  if (!normalized) {
    return false;
  }
  return !NON_WORKING_SHIFT_CODES.has(normalized);
}
...
function computeShiftSnapshot({
  ...
  breakMinutes = 0,
  breakIncluded = false,
  isRealWorkingShift = null,
  holidayResolver = () => ({ isHoliday: false }),
}) {
  const hasExplicitWorkingFlag = !(isRealWorkingShift === null || isRealWorkingShift === undefined);
  const hasShiftCode = String(shiftCode || '').trim().length > 0;
  const isWorkingShift = hasExplicitWorkingFlag
    ? Boolean(isRealWorkingShift)
    : (hasShiftCode ? isRealWorkingShiftCode(shiftCode) : true);
  ...
  const normalizedIntervals = Array.isArray(intervals)
    ? intervals
      .map((interval) => {
        ...
        return normalizeShiftInterval(startMinutes, endMinutes);
      })
      .filter(Boolean)
    : [];
  ...
  if (!normalizedIntervals.length) {
    ...
    const fallbackInterval = normalizeShiftInterval(startMinutes, endMinutes);
    if (fallbackInterval) {
      normalizedIntervals.push(fallbackInterval);
    }
  }
  ...
  const durationMinutes = normalizedIntervals.reduce((total, [start, end]) => total + Math.max(0, end - start), 0);
  const workedMinutes = hasSplitIntervals
    ? durationMinutes
    : calcWorkMinutes(durationMinutes, breakMinutes, breakIncluded);
  const breakApplied = hasSplitIntervals
    ? 0
    : (Boolean(breakIncluded) ? 0 : Math.min(durationMinutes, Math.max(0, Number(breakMinutes) || 0)));

  const dayParts = buildSegments({ baseDateISO: dateISO, intervals: normalizedIntervals });
  ...
  const nightMinutes = normalizedIntervals
    .reduce((total, interval) => total + calcNightMinutes(interval), 0);
  const crossMidnight = normalizedIntervals.some(([start, end]) => end > MINUTES_PER_DAY);
